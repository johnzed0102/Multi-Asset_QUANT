...
INCLUDE_MACRO_INTERMEDIATE = False
...
def save_line_plot(series_dict: dict[str, pd.Series], title: str, ylabel: str, output_path: Path, hline: float | None = None) -> None:
...
def build_summary_markdown(perf_df: pd.DataFrame, turnover_df: pd.DataFrame) -> str:
    ...
    lines.append("我们对比了两套策略：Baseline 风险平价引擎与 Macro Stable 宏观驱动稳定版。")
    lines.append("在宏观版本中，我们让宏观信号只用于调整风险预算，再通过协方差结构映射为实际权重，不直接由宏观信号拍板持仓。")
    lines.append("")
    lines.append("## 2) 关键设定与约束")
    lines.append("我们使用月度调仓框架，并以风险预算/风险贡献思想构建多资产配置。")
    lines.append("我们设置 TARGET_VOL=0.05 作为展示参照线，用于观察滚动波动率是否贴近目标风险水平。")
    lines.append("对权重约束方面，我们在结果中观察到单资产与单步变动受到明显限制；若需声明为模型内硬约束，应以回测脚本实现为准。")
    lines.append("")
    ...
    lines.append(
        f"- 资金利用率（avg_cash）: Baseline={format_pct(baseline['avg_cash'])}, Macro Stable={format_pct(macro['avg_cash'])}"
    )
    lines.append(
        f"- 集中度（max_single_asset_weight）: Baseline={format_pct(baseline['max_single_asset_weight'])}, "
        f"Macro Stable={format_pct(macro['max_single_asset_weight'])}"
    )
    lines.append(
        f"- 单步变动（max_single_step_weight_change）: Baseline={format_pct(baseline['max_single_step_weight_change'])}, "
    ...
    if "baseline" in turnover_df.index and "macro_stable" in turnover_df.index:
        lines.append(
            f"- 换手率均值: Baseline={format_pct(turnover_df.loc['baseline','mean'])}, "
            f"Macro Stable={format_pct(turnover_df.loc['macro_stable','mean'])}"
        )

    lines.append("")
    lines.append("## 4) 宏观版波动/集中变化的解释（谨慎表述）")
    lines.append("宏观版本通过风险预算再分配改变了资产风险暴露结构，而非直接设定权重。")
    lines.append("当宏观信号持续偏向某类资产时，组合可能出现更高权益或信用暴露，从而引起波动、回撤与换手特征的阶段性变化。")
    lines.append("因此，在展示中应同时报告收益、波动、回撤、换手与现金占比，避免只看单一绩效指标。")
